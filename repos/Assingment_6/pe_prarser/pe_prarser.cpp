...
{
    wchar_t path_pefile[] =
        L"C:\\Users\\백다현\\source\\repos\\Assingment_6\\Release\\test_write.exe";
    ...
    // IMAGE_FIRST_SECTION(pNtHeader) 매크로 대신 수동 계산
    ...
    if (importDir.VirtualAddress == 0 || importDir.Size == 0) {
        ...
        // (a) IAT가 저장된 섹션 찾기 + RVA->RAW 계산 함수 없이 인라인 처리
        //     importDir.VirtualAddress 가 속하는 섹션을 찾아 RAW 오프셋 계산
        int importRaw = -1;
        ...
        // 헤더 영역일 가능성: RVA < SizeOfHeaders 이면 RAW==RVA 로 취급
        ...
        if (importRaw >= 0 && (DWORD)importRaw < dwSize) {
            ...
            for (int idx = 0; ; ++idx, ++pDesc) {
                ...
                // DLL 이름: pDesc->Name (RVA) -> RAW 변환 (인라인)
                int nameRaw = -1;
                ...
                // (c) Thunk 테이블: OriginalFirstThunk(있으면) 우선, 없으면 FirstThunk 사용
                ...
                // (d) 개별 thunk 순회 (32비트: 4바이트씩, 0이면 종료)
                ...
                while (off + 4 <= dwSize) {
                    ...
                    if (entry & 0x80000000u) {
                        ...
                        // name import: entry는 IMAGE_IMPORT_BY_NAME 구조체의 RVA
                        int ibnRaw = -1;
                        ...
                        if (ibnRaw >= 0 && (DWORD)(ibnRaw + 2) < dwSize) {
                            // [2바이트 hint] + 함수명(ASCII, null-terminated)
